...
public class Page_Home_Admin_LicenseDetails {
	...
	def enterSearchCriteria (String criteria) {
		KeywordUtil.logInfo("Complete Search parameter: " + criteria)

		WebUI.waitForElementVisible(inputSearchCriteria, 40)

		WebUI.sendKeys(inputSearchCriteria, criteria)

		KeywordUtil.markPassed("Search parameter was completed: " + criteria)
	}


	@Keyword
	def selectFirstNameForSearch () {
		KeywordUtil.logInfo("Select search criteria: First Name")
		...
		KeywordUtil.markPassed("Search criteria was selected: First Name")
	}
